...
public class OrdersController {
    ...
    public R update(@RequestBody OrdersEntity orders, HttpServletRequest request){
        if (StringUtils.equals(orders.getStatus(), "已退款")) {
            String[] address = orders.getAddress().split(",");
            HashSet hashSet = new HashSet(Arrays.asList(address));
            Long goodid = orders.getGoodid();
            DianyingxinxiEntity dianyingxinxiEntity = dianyingxinxiService.selectById(goodid);
            String[] split = dianyingxinxiEntity.getSelected().split(",");
            ...
            dianyingxinxiEntity.setSelected(stringBuffer.toString());
            dianyingxinxiService.updateById(dianyingxinxiEntity);
        }
